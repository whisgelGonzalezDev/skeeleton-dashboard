...
const ROLE_STYLES: Record<UserRole, string> = {
  Admin:
    'text-amber-700 bg-amber-50 border border-amber-200 dark:text-amber-400 dark:bg-amber-950 dark:border-amber-900',
  Editor:
    'text-sky-700 bg-sky-50 border border-sky-200 dark:text-sky-400 dark:bg-sky-950 dark:border-sky-900',
  Viewer:
    'text-zinc-600 bg-zinc-100 border border-zinc-200 dark:text-zinc-400 dark:bg-zinc-800 dark:border-zinc-700',
}
...
function StatusDot({ status }: { status: UserStatus }) {
  ...
  return (
    <span className="flex items-center gap-1.5">
      <span
        className={`h-1.5 w-1.5 rounded-full ${active ? 'bg-emerald-500' : 'bg-zinc-300 dark:bg-zinc-600'}`}
      />
      <span
        className={`text-xs font-medium ${active ? 'text-emerald-700 dark:text-emerald-400' : 'text-zinc-400 dark:text-zinc-500'}`}
      >
        {status}
      </span>
    </span>
    ...
    <tr className="border-b border-zinc-100 dark:border-zinc-800">
      {[55, 70, 30, 25, 40].map((w, i) => (
        <td key={i} className="px-4 py-3.5">
          <div className="h-3.5 rounded skeleton-shimmer" style={{ width: `${w}%` }} />
        </td>
      ))}
    </tr>
  )
